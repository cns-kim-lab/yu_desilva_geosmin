...
def plot_dose_dp_PER(ax,data,cidx=0,alpha=1,linestyle='solid',shape='o',**fig_params):
    ...
    ct = fig_params['ct']
    p1 = ax.errorbar(list(range(len(data.columns))),data.values.mean(axis=0),yerr=std_err(data.values),linewidth=lw,elinewidth=elw,markersize=ms,capsize=cs,color=cmap,ecolor=ecolor,capthick=ct,alpha=alpha,linestyle=linestyle,marker=shape)  
...
def draw_paired_bar_figure(
    ...
    for group_idx, group in enumerate(groups):
        ...
        ax.bar(
            ...
            capsize=capsize,
            error_kw={
                'elinewidth': elinewidth,   # error bar 선 굵기
                'capthick': capthick,     # cap 굵기
                'ecolor': 'black'    # 색상
            }
        )
